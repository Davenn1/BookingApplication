import './SeatPage.css';
import KursiAvailable, { datase } from './kursiAvailable';
import KursiNotAvailable from './kursiNotAvailable';
import { useContext, useState, createContext, useEffect } from 'react';
import { count } from './kursiAvailable';
...
function SeatPage(props) {
  const { title, room, count } =  useParams();
  limit = count;
  ...
  return (
    ...
    <p className="studio">Studio {room}</p>
  ...
  <div id="myModal" className={`modal2 ${display}`}>
